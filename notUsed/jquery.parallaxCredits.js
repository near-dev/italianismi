...
window.setTimeout(function() {$('#cr_layer1, #cr_layer2').css(edge, '0px'); $('#entra, #cominciare, #lang').css('opacity', 1); }, 100); 
    ...
    if (_lastMouseDownXY-_mouseDownXY < 0) {
      ...
      if (o.LAYER.length>0) {
        $.each(o.LAYER, function(index, value) {
          $('#cr_layer'+(index+1)).css(edge,(sliderLT + Math.ceil(_velocity + bouncing))/value); //layer
        });
      }
    } else {
      ...
      if (o.LAYER.length>0) {
        $.each(o.LAYER, function(index, value) {
          $('#cr_layer'+(index+1)).css(edge,(sliderLT + Math.floor(_velocity + bouncing))/value); //layer
        });
      }
    }
}};
...
var touchMove=function(e) { //mouse move
  ...
  if (_mouseDown) {
    ...
    if (o.LAYER.length>0) {
      $.each(o.LAYER, function(index, value) {
        $('#cr_layer'+(index+1)).css(edge, (_mouseDownLT + (MouseXY - _mouseDownXY))/value); //layer
      });
    }
